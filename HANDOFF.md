...
- **Tier-2 B (in-terminal Globus login) is BUILT** on `feat/in-terminal-login` (PR #48): `needs_login` phase gating `connect_facility` first; `authenticate(force, mode)` / `complete_login(code)` tools; loopback browser flow + paste-back fallback (headless-safe); minimum consent (Search reads are anonymous). L1–L4 live-validated, independent review done and its fixes in. Next: merge #48 → Tier-2 A (default registry index, `list_facilities` out of the box, 'add your facility' docs). Design + decisions: `Planned/In-terminal Globus login.md`.
