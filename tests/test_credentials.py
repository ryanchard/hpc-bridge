...
def _seed_source(path: Path, records: dict[str, TokenStorageData], namespace=NS) -> None:
    src = SQLiteTokenStorage(filepath=path, namespace=namespace, connect_params={"check_same_thread": False})
    src.store_token_data_by_resource_server(records)
    src.close()


def _read(path: Path, namespace=NS) -> dict[str, TokenStorageData]:
    st = SQLiteTokenStorage(filepath=path, namespace=namespace, connect_params={"check_same_thread": False})
...
def test_build_raises_when_required_token_missing(tmp_path):
    ...
    with pytest.raises(MissingCredentials, match="auth.globus.org"):
        build_minimal_storage_db(src_path=src, dst_path=tmp_path / "out.db", namespace=NS)


def test_build_raises_when_refresh_token_absent(tmp_path):
    src = tmp_path / "src.db"
    _seed_source(src, {
        "funcx_service": _td("funcx_service", refresh=None),  # no refresh -> dies at expiry
        "auth.globus.org": _td("auth.globus.org"),
    })
