...
python3 - "$CLAIMS" "$USER_" "$KEY" "$USER_@$HOST" "$REMOTE" <<'PY' || { rc=$?; [ "$rc" = 2 ] && echo "refusing: $USER_ is claimed by a live harness process"; exit "$rc"; }
import fcntl, os, subprocess, sys
d, u, key, dest, remote = sys.argv[1:6]; os.makedirs(d, exist_ok=True)
fd = os.open(os.path.join(d, f"{u}.lock"), os.O_RDWR | os.O_CREAT, 0o644)
try: fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
except OSError: sys.exit(2)
r = subprocess.run(["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=20", "-i", key, "-o", "IdentitiesOnly=yes", dest, remote])
