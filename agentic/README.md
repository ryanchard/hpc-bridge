...
- ✅ **First measurement — skill ablation (2026-07-07, n=20, $13.75) — corrected by re-grade:** the initial 5/5 → 2/5 delta on `happy_path` was a **grader miscalibration** (the no-raw-SSH invariant anchored too early; the flagged `login_shell` calls were *pre-endpoint*, the sanctioned escape hatch). Corrected graders: **no invariant-level delta** (5/5 vs 5/5, both scenarios). The behavioural difference stands, non-violating: 3/5 ablated runs did raw-SSH discovery pre-endpoint vs 0/5 baseline — the skill steers discovery onto the endpoint channel. The correction itself came from `harness/regrade.py` replaying stored bundles through the fixed invariants: **grading is re-runnable evidence, and it caught our own false finding.**
