...
## Status
- ✅ **Grading core** (`invariants.py` + `test_invariants.py`) — deterministic invariants, **7 unit tests green**, pure/hermetic (no SDK, no cluster). Run: `uv run pytest agentic/harness/test_invariants.py -q`.
...
- ✅ **Suite runner** (`run_suite.py`) — staggered (rate-limit-safe), capped (≤10, a distinct pool user per slot), matrix over **scenario × model × effort × repeat**; aggregates pass/fail **per `model @ effort`**, so you measure model capability *and reasoning level* against workflow behaviour (the invariants are the dependent variable). `run.py`/`run_smoke.sh` gained `--model`/`HPCB_MODEL` and `--effort`/`HPCB_EFFORT` (reasoning `low|medium|high|xhigh|max`, paired with adaptive thinking). Example: `python agentic/run_suite.py --scenarios happy_path --models claude-opus-4-8,claude-sonnet-4-6 --efforts low,high,max --repeat 3`.
...
- ⏳ **Next:** the skill-ablation measurement (`--ablations none,skill --repeat 5`) · LLM-judge (fed from `runs/` bundles — offline re-grading) · reuse: hpc-bridge `reused` signal + setup→reuse chaining · **cost-gating** (Plan A — makes the gate *rich*) · faithful plugin/skill loading.
