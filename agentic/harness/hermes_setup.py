"""Write ~/.hermes/config.yaml for the jail's hermes operator: the ALCF (OpenAI-compatible) model provider plus
hpc-bridge as an MCP server. hermes filters the environment for stdio MCP servers to a static `env:` allowlist in the
config, but the jail injects the scoped hpc-bridge credentials at RUN time — so we generate the config here, at
container start, folding the current env into the mcp_servers env block.

The ALCF bearer is NOT written to the file: `api_key: ${ALCF_INFERENCE_TOKEN}` is interpolated by hermes from the env
(minted on the host, passed with `-e`). We deliberately do NOT set HPC_BRIDGE_OMIT_INSTRUCTIONS for hpc-bridge here —
hermes has no skill system, so it SHOULD get the guidance pointer + resource (unlike Claude Code, which opts out).
"""
...
# NB: HPC_BRIDGE_OMIT_INSTRUCTIONS is intentionally excluded — hermes wants the guidance.
_PASSTHROUGH = (
...
def main() -> int:
    cfg = build_config()
    out = Path.home() / ".hermes" / "config.yaml"
