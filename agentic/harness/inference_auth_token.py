# Vendored from argonne-lcf/inference-endpoints (MIT License), unmodified.
#   https://github.com/argonne-lcf/inference-endpoints/blob/main/inference_auth_token.py
# The ALCF inference service auth helper: a Globus native-app OAuth flow that caches access+refresh
# tokens under ~/.globus/app/ (NEVER in this repo) and refreshes the 24h access token on demand.
# Used by alcf_inference.py to drive open-source harnesses with ALCF-hosted models in the agentic tier.
# Only dependency is globus-sdk, already a hpc-bridge dependency. Do not edit here; re-vendor from upstream.
