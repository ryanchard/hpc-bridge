...
def get(name: str | None = None) -> Target:
    ...
    if name == "globus1":
        return Target(
            ...
            cleanup_host="globus1.cs.uchicago.edu", cleanup_ssh_opts=(),
        )
    if name == "fake":
        ...
        return Target(
            name="fake", ssh_host="login", nodes=2, endpoint_prefix="hpc-bridge-fake",
            default_key=key, docker_network=os.environ.get("HPCB_FAKE_NETWORK", "hpcb-fake_default"),
            # the login container's sshd on the host port, as a pool user (host keys churn per image build)
...
def main(argv: list[str]) -> int:
    ...
    for k, v in (("NAME", t.name), ("SSH_HOST", t.ssh_host), ("NODES", t.nodes), ("EP_PREFIX", t.endpoint_prefix),
                 ("KEY_DEFAULT", t.default_key), ("NETWORK", t.docker_network or "")):
        print(f"HPCB_T_{k}={v}")
    return 0
