...
- **The fallback feeds the happy path (write-back).** A canary-validated Socratic stand-up has hand-built an index entry; `provenance: plugin-validated` is the hook (review-then-publish). The index **grows as compute comes online**, from real validated stand-ups.
