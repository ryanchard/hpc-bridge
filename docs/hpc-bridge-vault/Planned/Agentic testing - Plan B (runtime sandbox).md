...
- **Missing-affordance bug → deterministic red scenario.** The behaviour is impossible today because a tool field/notice doesn't exist, so *no* agent behaviour can pass it. Example: `endpoint_reuse` (issue #20) — the second `connect_facility` must carry `reused: true`; RED live (10/10), unit-checked GREEN-with-the-field. Reuse actually happens server-side (session cache) but is invisible in the result — the exact gap the field closes.
