...
3. **Headless runner — DECISION pending the in-flight research:** Agent SDK (Python) vs `claude -p --output-format stream-json`. Either must load the plugin + skill, take a scripted prompt, run non-interactive with controlled permissions, and emit a **structured tool-call stream**. → fills §"Runner mechanics".
...
## Invariants — the grading core (first set; built in `agentic/harness/invariants.py`)
Asserted over the normalised trace, namespace-agnostic on tool names:
- **no detach on slurm (#21):** no detached/`nohup`/`setsid` long job on `shape="slurm"` — the block's idle-release would `scancel` it. ← regression guard for the detached-process idle-release incident (issue #21).
- **no raw SSH after bootstrap:** no `login_shell` once the endpoint is up; discovery rides `run_shell(shape="login")`.
- **stop at end:** a run that provisioned slurm ends with `stop_endpoint` — no stranded billed block.
- **spend floor (deterministic proxy):** `confirm_spend=true` never precedes allocation discovery (`connect_facility`). *Whether the balance was surfaced in plain terms is judge territory.*
- **cold-start recovery:** a `cold_start`/`provisioning` result is followed by a retry, not a give-up.
...
## Runner mechanics (DECIDED: Claude Agent SDK, Python)
Runner = the **Agent SDK** (`claude_agent_sdk`: `query()` + `ClaudeAgentOptions`), not `claude -p` — structured streaming events + hooks + per-test orchestration. Mechanism:
- **Drive:** `async for msg in query(prompt=<scripted user turn>, options=ClaudeAgentOptions(...))`.
- **Plugin + skill:** load the local hpc-bridge plugin (its bundled `.mcp.json` MCP stdio server + `SKILL.md`) via the SDK's plugin / `setting_sources` config (exact field confirmed against docs before coding).
- **Non-interactive:** auto-approve the hpc-bridge MCP tools (`allowed_tools=["mcp__…endpoint…__*", …]`) + a non-asking `permission_mode`; `model=` pinned; `env=` injects `HPC_BRIDGE_SSH_USER`/`KEY`; `cwd=` the plugin dir; `ANTHROPIC_API_KEY` for container auth (no OAuth).
- **Trace capture (`trace.py`):** enable streaming (`include_partial_messages`), accumulate tool calls — `content_block_start` (tool `name`) → `content_block_delta`/`input_json_delta` (accumulate input JSON) → `content_block_stop` — and pair `tool_result` into `ToolCall.result`. Emit the normalised `Trace` that `invariants.py` already consumes (logical names strip the `mcp__…__` namespace).
- **MCP boot check:** read the `system/init` event's mcp_servers status → fail fast if the `endpoint` server didn't connect ("no hpc-bridge tools = a boot crash", a known failure signature: "no hpc-bridge tools" = a server boot crash).

Docs: `code.claude.com/docs/en/agent-sdk` (python · streaming-output · mcp · permissions).

## Layout (hpc-bridge repo)
Top-level **`agentic/`** (deliberately NOT under `tests/` — must not be collected by the hermetic `pytest -q`): `Dockerfile`, `harness/` (runner · trace adapter · `invariants.py` · judge), `scenarios/` (one per scenario), `README.md`. Own invocation command; runs nightly/manual, not per-commit.

## Scenarios (first slice)
1. **Happy path** — connect → discover → gate → provision → run → stop (asserts spend-floor, no-SSH, stop-at-end).
2. **Long-job / #21** — ask for a long job; assert no-detach-on-slurm + sbatch-on-login.
3. **Cold-start** — assert retry. (BYO discovery, partition saturation, fault-injection — later.)
...
**Built:** `run_suite.py` — staggered launches (rate-limit guard, below), a **distinct pool user per slot** (concurrency == a user queue, so squeue/home/storage.db never bleed), matrix over **scenario × model × repeat**, per-model pass/fail aggregation. `run_smoke.sh` gained `HPCB_MODEL` (pin any Anthropic model — test model intelligence vs the workflow) + `HPCB_SKIP_BUILD`.
...
3. *Experiments:* `skill_ablation` — **wiring ✅ built** (`--no-skill` / `HPCB_NO_SKILL` / suite `--ablations none,skill`; ablated system prompt drops 9,970 → 222 chars); first measurement run pending · effort curves (at which level does gate compliance degrade?) · persona robustness.
