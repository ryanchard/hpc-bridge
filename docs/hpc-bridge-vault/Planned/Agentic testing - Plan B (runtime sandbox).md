...
2. *Capability:* `endpoint_reuse` (intra-agent) + `endpoint_reuse_chain` (inter-agent, cross-restart) — **✅ BUILT + GREEN ([#20](https://github.com/ryanchard/hpc-bridge/issues/20))**: hpc-bridge now surfaces `reused` in the connect result · `byo_bad_host` (unreachable ⇒ ask, don't invent) · `config_correction` (human corrects `interface` ⇒ the correction lands in `details`).
3. *Experiments:* `skill_ablation` — **✅ measured across two sweeps; finding refined twice by evidence (2026-07-07).**
   - *Sweep 1 (n=20, $13.75):* first read `happy_path` 5/5 → 2/5. **Overturned by the post-review re-grade** — a grader miscalibration (the flagged `login_shell` calls were **pre-endpoint**, the sanctioned escape hatch). The re-grade loop caught our own false finding.
   - *Sweep 2, corrected graders (n=32, $12.91):* `happy_path` baseline **8/8** vs ablated **6/8** — and the failures are the interesting part. Both were **world-check catches**: `stop_endpoint` returned `status="down"` but its notice admitted *"cancel not confirmed (allocating nodes…); idle-release will reclaim it"* — the block was still burning at postcheck. Root cause chain, fully evidenced in the bundles: **8/8 baseline runs polled `squeue` via the login shape just before stopping (the SKILL's step-4 habit) → release channel warm → 8/8 cancels confirmed; 0/8 ablated runs did → login worker idle-released → 3/8 cancels UNCONFIRMED → 2 blocks left to the 600s idle-release net.** So the skill's measured value here is **cost-hygiene via channel warmth**, not gate compliance. Plus the standing observation: 3/8 ablated used pre-endpoint raw SSH vs 0/8 baseline.
   - **Plugin gap exposed (prime TDD candidate):** `stop_endpoint` reports `status="down"` even when the cancel is unconfirmed — the status contradicts its own notice. Spec-by-scenario: an honest status (`draining`/retry-until-confirmed/pre-warm the release channel) would make the ablated behaviour safe by construction.
   - *Also validated by sweep 2:* the vacuous-pass gates — **11 runs died instantly on a subscription 429** ("session limit") and every one was correctly FAILed by `agent_engaged`/`run_completed` (all 11 would have graded OK before the review fixes). The `gated_provision` cells were eaten by that 429 tail (baseline 4/4 valid passes; ~skill zero valid runs) — **re-run after the window reset**.
   - *Gated re-run (n=16, $12.13, post-reset — completes the dataset):* `gated_provision` baseline **8/8** (8/8 confirmed cancels, 8/8 login-warm before stop — the channel-warmth mechanism replicated on the interactive path) vs ablated **6/8**. The two ablated failures are new *mechanisms*: one hit a `cold_start`, never retried, never delivered compute, and **walked away without stopping the billed block** (world check caught it still running); the other simply never delivered the approved work (`compute_ran`). **Final corrected ablation: baseline 16/16, ablated 12/16** — the skill's measured value spans cost-hygiene (channel warmth), cold-start persistence, and delivery follow-through; the spend *gate* itself held even ablated.
   - Wiring: `--no-skill` / `HPCB_NO_SKILL` / suite `--ablations none,skill`. Next: section-level ablation · effort curves · persona robustness.
...
## TDD: scenarios as executable specs (2026-07-08)

Once the harness is trusted, development inverts: **write the failing scenario first, then grow the plugin until it greens.** Two shapes, and the distinction is the load-bearing lesson (learned the hard way — a first attempt to *manufacture* a race deterministically failed):

- **Missing-affordance bug → deterministic red scenario.** The behaviour is impossible today because a tool field/notice doesn't exist, so *no* agent behaviour can pass it. Example: `endpoint_reuse` (issue #20) — the second `connect_facility` must carry `reused: true`; RED live, then **GREEN live** once the signal shipped, with `endpoint_reuse_chain` proving the inter-agent (cross-restart) case too. Reuse actually happens server-side (`find_online_endpoint` over the web — *not* a config cache) but was invisible in the result — the exact gap the field closed.
- **Race-condition / property bug → universal invariant + statistical verification, NOT a bespoke scenario.** When the failure is a timing race with no manufacturable precondition, asserting "it never happens across the natural distribution" is the right test; trying to force the state is a fool's errand. Example: `stop_is_honest` (issue #24) — `stop_endpoint` returns `status="down"` while its own notice admits *"cancel not confirmed"* (the login worker scaled in before the AMQP `scancel` dispatch; measured **~5% of stops, 3/54 stored — all in the cold-login `happy_path ~skill` cell**, none manufacturable). It's a **universal invariant, reported on every run but deliberately NOT in any `EXPECT_OK`** until the fix lands (else it flakes the suite). The "before" baseline is that 5% measured from stored bundles; **green = fresh runs at 0 violations** (old bundles keep the old behaviour baked in and can't prove the fix). A bespoke `stop_confirmed` scenario was built, then **deleted** on this finding.

Verification note: `regrade.py` re-grades stored bundles for *grader* changes; a *plugin* fix can only be proven by fresh runs (bundles record the server's behaviour at capture time).
...
- **Reuse scenarios (stateful) — ✅ built ([#20](https://github.com/ryanchard/hpc-bridge/issues/20)).** A second `connect_facility` **reuses** a still-online endpoint (zero SSH) instead of re-bootstrapping — internal to hpc-bridge, so invisible in the agent's tool calls until we surfaced it. Both halves shipped: (a) hpc-bridge **surfaces reuse in the `connect_facility` result** (`reused: true` + notice) so an invariant can assert it, and (b) the harness gained a **`PHASES` chain primitive** (`run.py._run_chain`): each phase is a separate agent session (fresh MCP server = the "restart"), sharing the run's facility id with no teardown between, so a later phase reattaches to an earlier phase's endpoint. `endpoint_reuse` covers intra-agent; `endpoint_reuse_chain` covers inter-agent within one container.
- **Automated pull-down — ✅ built.** `run.py._teardown` runs `gce stop`+`delete <name>` over SSH as the test user (control-plane, off the hot path; command validated live) in a `finally`, so a run cleans up even on failure. **Per-scenario configurable:** `TEARDOWN = delete | keep`, plus an optional stable `FACILITY_ID` so a reuse chain shares one endpoint name (vs the default per-run-unique `globus1-<runid>`). Still to build: a **cross-container** suite chain (leave an endpoint up in one container, reuse it from a separate run) — the in-process `PHASES` chain already covers the cross-restart case *within* one container; a cross-container chain would set `TEARDOWN="keep"` + a stable `FACILITY_ID` so run 2 finds run 1's endpoint.
