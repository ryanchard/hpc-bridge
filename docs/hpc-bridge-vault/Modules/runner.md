...
- **`run(command, timeout=None)`** — submits a `ShellFunction(command)` and returns its `{returncode, stdout, stderr}`. Commands are escaped via `_escape_for_shellfunction` (`:40`) because `ShellFunction` runs `cmd.format()` (bare `{}`/`}` would break formatting). `timeout` overrides the runner default for one call — used by the teardown block-release to bound it tightly.
...
> A *secondary* bound: `run` does `await asyncio.to_thread(fut.result, timeout)`. A **running thread can't be cancelled**, so wrapping the call in `asyncio.wait_for` does *nothing* — it waits for the thread anyway. The only hard ceiling is the `timeout` arg to `fut.result` (`concurrent.futures` honors it). So a caller needing a short dispatch bound (e.g. the teardown block-release `scancel`, for the rare case its login worker idle-released) MUST pass `run(..., timeout=…)`, never `wait_for`.
