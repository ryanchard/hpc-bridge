...
- **Facility selection.** `make_facility` (`:70`) returns a `SlurmFacility` (`HPC_BRIDGE_FACILITY=anvil`) or a `LocalFacility`, reading the login-node pin from [[state]] and rebinding the CLI to it.
