...
- **The lock.** Serialises provision / runner-swap / teardown so concurrent tool calls can't race `AppCtx`. Dispatch happens *outside* the lock, so a long command doesn't serialise everything else. `stop_endpoint` caps teardown at `STOP_TIMEOUT_S` (90s) so a slow login node can't hold the lock — and the whole server — hostage.
