...
- **`ConnectFacilityResult`** (`:119`) — result of `connect_facility`: `phase` (`needs_account` | `provisioning` | `needs_facility_details` | `proposed_facility_details` | `unsupported` | `failed`), `facility`, `reused` (True when the connect reattached to an already-online endpoint — a zero-SSH reconnect, [[Standing up the endpoint]], [#20](https://github.com/ryanchard/hpc-bridge/issues/20)), `allocations` (`AllocationOption`s — account + balance), `proposed_details` (the discovered `FacilityDetails` draft, set on the `proposed_facility_details` phase), `notice`. Drives the [[Facility catalog|catalog]] selection loop.
