...
- **`ConnectFacilityResult`** (`:133`) — result of `connect_facility`: `phase` (`needs_account` | `provisioning` | `needs_facility_details` | `proposed_facility_details` | `needs_preauth`, `needs_login` (Globus login gate: `login_url` + `login_mode` browser|paste — a phase, not a prompt; `LoginStatus` is the same shape for the `authenticate`/`complete_login` tools) | `unsupported` | `failed`), `facility`, `reused` (True when the connect reattached to an already-online endpoint — a zero-SSH reconnect, [[Standing up the endpoint]], [#20](https://github.com/ryanchard/hpc-bridge/issues/20)), `allocations` (`AllocationOption`s — account + balance), `proposed_details` (the discovered `FacilityDetails` draft, set on `proposed_facility_details`), `preauth_command` (the `ssh -fN …` line the **user** runs in their own terminal when `phase="needs_preauth"` — an MFA/password host; the secret never reaches the agent, [[MFA and interactive SSH auth]]), `notice`. Drives the [[Facility catalog|catalog]] selection loop.
...
The `needs_confirmation` phase/status is the [[Resource shapes & the spend floor|spend floor]] signal; `cold_start` is the no-hang cold path; `running` is a long task's poll handle ([[The MCP tools|poll_task]]); `proposed_facility_details` (a probed draft to confirm), `needs_facility_details` (no SSH host yet), and `needs_preauth` (the host needs an interactive login) are the un-indexed-facility signals.
