...
`discover_facility_details(target)` (`discovery.py:48`) runs **one** batched `ssh_exec` over a bare [[facility-remote|SshTarget]] — just an `ssh_host` plus `~/.ssh/config` credentials, with **no endpoint and no catalog entry needed yet** — then `parse_probe()` (`:59`) turns the output into a `(FacilityDetails, notes)` pair. `notes` names the low-confidence fields the agent must confirm with the user — `interface` above all.

## How it works

- **One probe, framed.** `_PROBE` is a single compound bash script (one SSH round-trip ⇒ MFA-once-friendly) emitting `KEY=value` lines between `HPCB_PROBE_BEGIN`/`END` sentinels (so a login banner can't pollute the parse); multi-valued facts (`PART`/`NIC`) repeat their key. `_collect()` reads only the framed block.
- **Deterministic per-field proposers** (never model inference): `_interface()` picks the worker NIC from `ip -o -4 addr` (prefers a dedicated fabric `ib*`/`hsn*`, else the single candidate — *always* flagged); `_default_partition()` from `sinfo` (prefers a cheap `debug`/`shared`, else the Slurm default); scratch from `$SCRATCH`/`$WORK`/`$HOME` (templated to `{user}`); `_env_setup()` returns an activate line if gce is already installed, else the idempotent **uv create-venv + install** one-liner (`_UV_ENV_SETUP`) when `uv` is present; `_allocation()` detects `mybalance`/`xdusage`.
- **`{user}` templating** comes from the probe's own `whoami`, so the draft is per-user with no env var.

Reached from `connect_facility`'s index-miss path: `_propose_or_ask` ([[server]] `:796`) builds the bare target and returns `phase="proposed_facility_details"`.
