...
- **`provision(profile)`** (`:41`) — `configure` (forced `--multi-user false`), write the engine into the UEP template, `start`. Drives the local [[endpoint|`EndpointCLI`]].
- **`manager_online(endpoint_id)`** (`:51`) — the Globus web status query.
