...
- **`SearchCatalog`** (`catalog/search.py`) — live `get_subject` → write-through cache → bundled fallback. The *happiest* channel.
- **`BundledCatalog`** (`catalog/bundled.py`) — the checked-in seed YAML, shipped in the wheel: offline fallback + ingest source + zero-network test fixture.
- **`FakeCatalog`** (`tests/fakes.py`) — in-memory test double.

`make_catalog()` ([[server]]) selects from env (`HPC_BRIDGE_SEARCH_INDEX` → Search, else Bundled), mirroring `make_facility()`.

> [!note] Auth — reuse the Compute identity
> Reads use `SearchClient(app=Client().app)` — the same Globus identity Compute already holds — needing a one-time search-scope consent (`hpc-bridge-catalog`), and **falling back to the bundled seed** until granted. This unlocks `visible_to`-restricted entries. See [[Globus index discovery channel]].
