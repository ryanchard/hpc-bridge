...
- **The catalog drives the shape.** A Globus Search index (or the bundled seed) holds the facility shape; the **[[Facility catalog|catalog resolver]]** turns an entry into a `MachineProfile` — at startup (`HPC_BRIDGE_MACHINE`) or at runtime (`connect_facility`). There is no hardcoded machine profile — the bundled seed is the anvil config. See [[Globus index discovery channel]].
