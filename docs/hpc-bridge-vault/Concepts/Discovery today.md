...
> What the plugin discovers *now*: after the one-time bootstrap, the agent probes the facility through the **login shape over AMQP** (no SSH). The per-facility shape comes from a **[[Facility catalog|catalog]]** (the Globus index, or the bundled seed), and machine + allocation are **agent-selected at runtime** (`list_facilities` → `connect_facility` → pick an allocation). The hardcoded `anvil_profile` survives only as the env-fixed fallback.
...
- **The catalog drives the shape.** A Globus Search index (or the bundled seed) holds the facility shape; the **[[Facility catalog|catalog resolver]]** turns an entry into a `MachineProfile` — at startup (`HPC_BRIDGE_MACHINE`) or at runtime (`connect_facility`). The hardcoded `anvil_profile` ([[facility-remote]] `:109`) remains only as the `HPC_BRIDGE_FACILITY` fallback. See [[Globus index discovery channel]].
- **Agentic machine + allocation selection.** `list_facilities` browses the catalog; `connect_facility(machine)` brings up the free login shape, runs the allocation command (e.g. `mybalance`) over Compute, parses it in code, and returns the allocations to pick from — the choice flows into `ensure_endpoint_up(account=…)`. → [[The MCP tools]]
