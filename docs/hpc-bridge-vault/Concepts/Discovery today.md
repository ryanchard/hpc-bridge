...
> What the plugin discovers *now*: after the one-time bootstrap, the agent probes the facility through the **login shape over AMQP** (no SSH), and the per-facility shape is still a **hardcoded `anvil_profile`** — a Globus index exists but the resolver that would replace the hardcode isn't wired yet.
...
- **The Globus index exists.** A public Globus Search index holds the facility shape (host, scheduler, interface, env_setup…) and is queryable anonymously via `globus_sdk.SearchClient` — but `make_facility` still builds a **hardcoded `anvil_profile`** ([[facility-remote]] `:109`); the resolver that turns an index entry into a `MachineProfile` is designed, not built.
