...
`stop_endpoint` is the explicit exit: it **`scancel`s the block over the warm login shape (AMQP)** — fast, the same channel as the polls — then `gce stop`s the manager over SSH (bounded, since a fresh SSH to a loaded login node is slow), and resets session state.
