...
| `authenticate(force=False, mode=None)` | `LoginStatus` | The Globus login gate as a tool: `logged_in`, or `needs_login` with a `login_url` — browser loopback flow (opens the browser; the same URL is returned to show) or Globus's paste-back auth-code page when headless / `mode="paste"`. 10-min TTL; `force=True` re-consents. Minimum consent = the Compute SDK's endpoint floor; Search is not requested (registry reads are anonymous). |
