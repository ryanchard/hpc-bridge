...
| **SDSC Expanse** (`expanse`, ACCESS) | SSH bootstrap on `login.expanse.sdsc.edu`, with a one-time code | Slurm | every login needs a TOTP code even with a key, so the agent hands you an `ssh` command to open a shared session in your own terminal, then carries on. The first connect installs `uv` and Globus Compute into your home there (a minute or two). Account = your Expanse project id (e.g. `chi152`); default partition `shared`, `debug` for short tests. Validated 2026-09-04 (login shape). |
