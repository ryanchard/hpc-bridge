...
| **Purdue Anvil, facility endpoint** (`anvil-mep`, ACCESS) | facility-run endpoint, zero SSH | Slurm | the same machine as `anvil` reached without SSH; your ACCESS identity must map to your Anvil account. Compute-only; account = your ACCESS project id lowercased (`cis250223`, or `cis250223-gpu` for GPU partitions); default partition `shared`; GPU partitions need a GPU request (`scheduler_options: "#SBATCH --gpus-per-node=1"`). Validated 2026-09-04. |
