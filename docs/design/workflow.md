...
**SSH-once thread (in progress).** Parallel to the above, a principle is being enforced: SSH is a *one-time bootstrap*, not a channel — every new SSH risks an MFA re-auth, so all sensing/behavior must ride the endpoint (AMQP + Globus web), not a fresh SSH. **Keystone landed:** `bootstrap` now reuses an already-online endpoint via the web (`find_online_endpoint`) with zero SSH (relates #3 MFA, #8 gce-list→SDK). Follow-ups: move discovery (`sinfo`/`mybalance`/`squeue`) onto the **login-shape AMQP** channel (vs `login_shell` over SSH); an **endpoint-native wait** (poll `squeue` via the login shape / canary, not SSH); and **stale-reuse reconciliation** (re-bootstrap a reused endpoint that never warms).
