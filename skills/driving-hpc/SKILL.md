...
- **Don't foreground-`sleep` between polls.** Use your harness's wait/timer primitive (in Claude Code: a `Monitor` until-loop, or `run_in_background: true` on the wait) at a queue-appropriate cadence (~20–30 s) — not a tight loop, and don't hammer the endpoint.
