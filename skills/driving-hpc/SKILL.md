...
**0. Select the facility.** `list_facilities()` lists the catalogued HPC systems; pick one (or the user names it) and call `connect_facility(facility="<id>")` — the `facility` arg is the `id` or `subject` from the list (e.g. `"anvil"` or `"purdue:anvil"`). This **binds the facility and establishes the endpoint** — standing up, or **reusing with zero SSH**, the manager plus a free login-node worker — then runs the facility's allocation command over the endpoint and returns `needs_account` with your **allocations** (account + balance). It's the only step that may SSH, and only once. `provisioning` ⇒ the login node is still warming; call again shortly.
   - **`needs_facility_details` ⇒ the facility isn't catalogued.** Don't dead-end: gather its config from the **user with `AskUserQuestion`** — the `details` schema is your question list (`ssh_host`, `interface`, `env_setup`, `scratch_root`, `partition`, optional allocation command). **Only offer option values for the genuinely common fields** (`interface`: ib0 / hsn0; `scheduler`: slurm; `partition`: the facility's queues). For the **facility-specific** fields (`ssh_host`, `env_setup`, `scratch_root`) — things only the user knows — ask them to provide it; do **not** pre-fill a plausible-but-invented option (a guessed `env_setup` they accept just wastes a bootstrap). Then call `connect_facility(facility, details=…)`; the login-shape bring-up validates the values (a wrong `interface`/`env_setup` ⇒ the worker never registers). Use `{user}` or `$USER` in paths (both resolve to the SSH login name). SSH user + key come from the environment, not `details`. You only pass `details` once — re-calls remember it.
