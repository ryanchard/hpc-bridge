...
**0. Select the machine.** `list_facilities()` lists the catalogued HPC systems; pick one (or the user names it) and call `connect_facility("<machine>")`. This **binds the machine and establishes the endpoint** — standing up, or **reusing with zero SSH**, the manager plus a free login-node worker — then runs the facility's allocation command over the endpoint and returns `needs_account` with your **allocations** (account + balance). It's the only step that may SSH, and only once. `provisioning` ⇒ the login node is still warming; call again shortly. *(If a machine is pinned by env — `HPC_BRIDGE_MACHINE`/`FACILITY` — you can instead `ensure_endpoint_up(shape="login")` and read the balance yourself in step 1.)*
