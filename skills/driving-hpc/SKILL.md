...
**4. Wait THROUGH the endpoint.** Poll `run_shell("squeue -u $USER -h -o '%i|%P|%T|%r'", shape="login")` — over the network, no SSH — for the pilot's state (`PENDING`→`RUNNING`) and *why* it pends (`Priority`, `Resources`). Once `RUNNING`, call `ensure_endpoint_up(shape="slurm")` **once** to confirm the worker registered (`up`).
   ...
   - **A wait only happens if you END YOUR TURN.** Starting a wait in the *background* (e.g. a `run_in_background` sleep) does **not** pause you — the timer runs while you keep going. So if you start a background wait and then immediately poll again in the same turn, **no time passed**: you just busy-poll, and the timer fires uselessly later as a stray notification. The actual wait pattern is: kick off the delay → write a one-line status → **stop and end your turn.** The harness re-invokes you when the delay elapses; *that* next turn is when you re-poll. Use **one** wait mechanism, not two.
   - **Cadence:** relaxed and lengthening (the `squeue` reason tells you if it's seconds or minutes); don't re-call `ensure_endpoint_up` repeatedly while it's pending. (Queue state comes from the `run_shell` **tool**, not a local shell, so a local `until`/`Monitor` loop can't watch it.)
