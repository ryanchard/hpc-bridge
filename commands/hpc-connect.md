...
1. **The user named a facility, or gave a login host** → `connect_facility(facility="<id-or-name>"[, ssh_host="<login-host>"])`. This binds the facility and stands up — or **zero-SSH reuses** — the endpoint, then returns your allocations. `provisioning` ⇒ call again shortly; `needs_login` ⇒ give the user the `login_url` (their browser completes it; then call again — or `complete_login(code)` in paste mode); `needs_facility_details`/`needs_preauth` ⇒ follow the `driving-hpc` guidance.
